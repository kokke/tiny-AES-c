...
// Enable ECB, CTR, CMAC and CBC mode. Note this can be done before including aes.h or at compile-time.
...
static int test_generate_cmac(void);
...
int main(void)
{
	...
	test_decrypt_ecb() + test_encrypt_ecb() + test_generate_cmac();
...
static int test_generate_cmac(void)
{
...
#endif
    uint8_t mlen[] = {0, 16, 20, 64};
    uint8_t tag[AES_BLOCKLEN];
    uint8_t i;
    struct AES_ctx ctx;
    ...
    for (i = 0; i < sizeof(mlen) / sizeof(mlen[0]); ++i)
    {
      AES_CMAC_generate(&ctx, in, mlen[i], tag, sizeof(tag));
      if (0 != memcmp(tag, out[i], sizeof(tag)))
      {
        printf("FAILURE!\n");
        return(1);
      }
